                            ...
                            <tbody>
                                ...
                                <tr style="padding: 15px 0;">
                                    <td class="text-center">
                                        <p class="text-md font-weight-bold mb-0">{{ $subscription->id  }}</p>
                                    </td>
                                    <td class="text-center">
                                        <p class="text-md font-weight-bold mb-0">{{ $subscription->user->name }}</p>
                                    </td>
                                    <td class="text-center">
                                        <p class="text-md font-weight-bold mb-0">{{ $subscription->pricing_plan->name }}</p>
                                    </td>
                                    <td class="align-middle text-center" style="padding-top: 10px; padding-bottom: 10px;">
                                        <form action="{{ route('subscription.destroy', $subscription->id ) }}" method="POST" style="display:inline-block;">
                                            @csrf
                                            @method('DELETE')
                                            <a href="javascript:;" class="btn btn-delete text-md" data-toggle="tooltip" data-original-title="Delete subscription"
                                               onclick="if(confirm('Are you sure you want to delete this subscription?')) { this.closest('form').submit(); }">
